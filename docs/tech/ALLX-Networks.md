...
### 1.1 生成HTTP请求消息
URL：Uniform Resource Locator，统一资源定位符。URL根据http、ftp、file等协议进行分类，协议不同，URL格式也随之不同，Web浏览器根据这些不同的协议格式对URL进行解析，得到URL的各个元素。  
...
![公私钥加密通信](https://github.com/CHENGCHANGHU/allx-javascript/blob/master/flowcharts/tls.png)
